...
build-npm-docker:
	...
	docker run --name moving-objects-gui-build -v $(pwd)/moving-objects-gui/dist:/root/moving-objects-gui/dist --entrypoint '/bin/sh' moving-objects-gui-image -c 'cd moving-objects-gui && npm run build'
wrapper:
	gradle wrapper
build-app:
